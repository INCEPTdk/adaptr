...
substantial uncertainty and instability in performance metrics (except the ideal
design percentage, which is at 100% here, but would likely drop somewhat if more
simulations were conducted).

### Visualise trial results
We need non-sparse results for `plot_history()` (but *not* for `plot_status()`
or `plot_convergence()` presented above), so first we re-run `run_trials()`:
