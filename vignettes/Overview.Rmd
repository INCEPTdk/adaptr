...
```{r}
setup_cluster(NULL) # Replace NULL with number of parallel workers wanted
```

For practical purposes, this vignette does not use parallelisation (and thus
`NULL` is used in the call above), but it is generally recommended when using
`adaptr`.
...
```{r include=FALSE}
# Remove cluster to avoid R CMD check note
setup_cluster(NULL)
```
