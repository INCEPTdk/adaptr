...
specifications with no between-arm differences) by using the `calibrate_trial()`
function. 
...
the `cores` argument of `run_trials()` (further details in the `setup_cluster()`
and `run_trials()` function definitions). Most functions used to post-process,
extract, and plot results can similarly be run in parallel.
