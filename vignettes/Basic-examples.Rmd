...
patients have reached follow-up may differ from the result when an adaptive
analysis is stopped.
...
setup_trial_binom(
  ...
  # more patients are randomised than have follow-up data available at each look
...
In this final example, we specify different probability thresholds for
superiority and inferiority stopping rules at different adaptive analyses.
Varying probability thresholds may similarly be specified for stopping rules for
equivalence and futility. Importantly, all probability thresholds must be
specified such that each threshold is never stricter than the previous.
