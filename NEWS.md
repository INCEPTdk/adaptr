...
*   Added the `calibrate_trial()` function, which can be used to calibrate a
    trial specification to (approximately) obtain a desired value for a certain
...
*   More and improved parallelisation. The functions `extract_results()`,
    ...
    in parallel via the `cores` argument or as described below (note: some
    other functions have not been parallelised, as they were already fast and
    the time it took to copy data to the clusters meant that the parallelised
    versions of those functions were actually slower than the original ones,
    even when tested on results from 10-100K simulations).
    
*   The `setup_cluster()` function has been added. This function is used to
    setup and use the same parallel cluster throughout a session, avoiding the
...
*   Better (safer, more correct) random number generation. Previously, random
    number generation across different simulations was managed in an
    *ad hoc*-fashion to produce similar results sequentially and in parallel;
    while the influence of this *ad hoc*-approach was minimal, the package now
    uses the `"L'Ecuyer-CMRG"` random number generator (see `base::RNGkind()`)
    and appropriately manages random number streams across parallel workers, and
    also when run sequentially, ensuring identical results regardless of the use
    of parallelisation or not. **Important:** Due to this change, simulation
    results from `run_trials()` and bootstrapped uncertainty measures from
    `check_performance()` will not be identical to those generated by previous
    versions of the package. In addition, individual `trial_result` objects in a
    `trial_results` object returned from `run_trials()` no longer contain any
    individual `seed` values, but instead `NULL`.
...
*   Fixed a bug in `extract_results()` (and thus also in functions relying on
    ...
    method for multiple simulated trials) that led too incorrect total event
...
*   Values for `inferiority` must now be less than `1 / number of arms` if no
    ...
    could lead to simultaneous dropping of all arms simultaneously, which could
    previously cause `run_trial()` to crash.
...
*   The `setup_trial()` functions now accepts `equivalence_prob` and
    ...
    values of `1` makes stopping impossible, however, these values may be used
    in a sequence of thresholds to effectively prevent early stopping for
    equivalence/futility but allowing stopping later (similarly to how this was
    already possible for `inferiority` and `superiority`).
