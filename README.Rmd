...
Next, setup a trial specification using the general `setup_trial()` function, or
one of the special case variants using default priors  `setup_trial_binom()`
(used in this example) or `setup_trial_norm()`.
...
identical outcome in each arm), but can be used to calibrate any parameter in a
...
`calibrated_binom_trial$best_sims`. These can be summarised using several
functions (with many of these supporting different 'selection strategies' for
...
Several visualisation functions are included (all optional, and all requiring
the `ggplot2` package installed).

First, the convergence and stability of performance metrics may be visually
...
```
The status probabilities of the trial overall (or for specific arms) according
to trial progress may be visualised using the `plot_status()` function:
...
**Use stopping thresholds for simulations according to another scenario:**
...
```
Similarly, trial statuses for the scenario with differences may be visualised:
