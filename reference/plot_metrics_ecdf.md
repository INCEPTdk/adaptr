...
- metrics:
  ...
  include: `size`, `sum_ys`, `ratio_ys_mean`, `sq_err`, `sq_err_te`,
  `err`, `err_te`, `abs_err`, `abs_err_te`, (as described in
  [`extract_results()`](https://inceptdk.github.io/adaptr/reference/extract_results.md),
  with the addition of `abs_err` and `abs_err_te`, which are the
  absolute errors, i.e., `abs(err)` and `abs(err_te)`). All may be
  specified using either spaces or underlines (case sensitive). Defaults
  to plotting `size`, `sum_ys`, and `ratio_ys_mean`.
...
Note that the arguments related to arm selection and error calculation
are only relevant if errors are visualised.
